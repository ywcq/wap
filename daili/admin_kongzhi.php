...
//获取全部订单
$sql = "SELECT COUNT(money) FROM web_wan.codepay_order  where status='OK'";
$all_order = mysqli_query($my_conn,$sql) or die(mysqli_error());
$all_order = mysqli_fetch_array($all_order);
//获取全部充值金额
$sql = "SELECT SUM(money) FROM web_wan.codepay_order  where status='OK'";
...
//本月记录
$sql = "SELECT COUNT(money) FROM web_wan.codepay_order  where status='OK' and up_time>'$ksday' and up_time<'$jsday' ";
$month_order = mysqli_query($my_conn,$sql) or die(mysqli_error());
$month_order = mysqli_fetch_array($month_order);

//本月充值金额
$sql = "SELECT SUM(money) FROM web_wan.codepay_order  where status='OK'  and up_time>'$ksday' and up_time<'$jsday'   ";
...
//7天金额
$sql = "SELECT SUM(money) FROM web_wan.codepay_order  where status='OK'  and DATE_SUB(CURDATE(), INTERVAL 7 day) <= date(up_time)";
$day_7 = mysqli_query($my_conn,$sql) or die(mysqli_error());
$day_7 =  mysqli_fetch_array($day_7 );

//7天记录
$sql = "SELECT COUNT(money) FROM web_wan.codepay_order  where status='OK' and DATE_SUB(CURDATE(), INTERVAL 7 day) <= date(up_time)";
$day7_order = mysqli_query($my_conn,$sql) or die(mysqli_error());
$day7_order = mysqli_fetch_array($day7_order);


//总注册人数
$sql = "SELECT * FROM account.normal";
...
$js = 0;
$sql = "SELECT * FROM account.normal where create_time>'$ksday' and create_time<'$jsday' order by id desc";
$res = mysqli_query($my_conn, $sql);
...
-->	
  ...
  <ul class="panel quick-menu clearfix">
    ...
    <li class="col-sm-4">
      <a><i class="fa">充值管理代理注册链接</i>
      <input type="text" style="background: url(./images/blue_logo.png) no-repeat 5px 8px;text-indent: 15px;" class="share-input"  value="http://<?php echo $go.$reg;?>?t=<?php echo $user;?>" id="copy-content"/>
...
<div class="container-widget">
  ...
  <ul class="topstats clearfix">
    ...
    <li class="col-xs-6 col-lg-2">
      <span class="title"><i class="fa fa-users"></i>总注册人数</span>
     ...
     <div class="panel panel-widget" style="height:450px;">
        ...
        <div class="panel-body table-responsive">
          ...
          <table class="table table-dic table-hover ">
            <tbody>
			<?php
                $query="select * from web_wan.codepay_order where status='OK'  ORDER BY up_time DESC limit 10 ";
